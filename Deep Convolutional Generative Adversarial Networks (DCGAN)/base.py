...
class Trainer:
    ...
    def _make_barch_generator(self, ):
        transforms_train = transforms.Compose([
                           transforms.Resize(cfg.img_shape[:2]), # h, w
                           transforms.CenterCrop(cfg.img_shape[:2]),
                           transforms.ToTensor(), 
        ...
        if cfg.dataset == "MNIST":
            ...
            train_dataset = datasets.CelebA(root = cfg.dataset_dir, split = 'train', download = True, transform = transforms_train)

        self.data_loader = torch.utils.data.DataLoader(train_dataset, batch_size = cfg.batch_size, shuffle = True, num_workers = cfg.num_workers)
